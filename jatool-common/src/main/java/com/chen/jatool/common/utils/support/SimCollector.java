 ...
 * @since 5.6.7
 */
public class SimCollector<T, A, R> implements Collector<T, A, R> {
	/**
	 * 创建新的结果容器，容器类型为A
	 */
	 ...
	 */
	public SimCollector(Supplier<A> supplier,
	...
	public <R1> SimpleCollector<T,A,R1> andThen(Function<R, R1> finisher) {
		return new SimpleCollector<>(supplier, accumulator, combiner, this.finisher.andThen(finisher), characteristics);
	}
